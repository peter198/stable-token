...
contract Treasury is Ownable {
    ...
    function swapToken(bool _elcBuyElpTag, uint256 _amountIn) public returns (uint256){
        ...
        if(_elcBuyElpTag)
        {
            ...
            _amounts = BakerySwapLibrary.getAmountsOut(elcSwap.factory(),_amountIn, _path1);
            TransferHelper.safeTransfer(address(elc),address(elcSwap),_amountIn);    
        }else{
            _path1[0] = address(elp);
            _path1[1] = address(usdt);
            _path1[2] = address(elc);
            _amounts = BakerySwapLibrary.getAmountsOut(elpSwap.factory(),_amountIn, _path1);
        ...
        for (uint256 i = 0; i < _path1.length - 1; i++) {
            ...
            (address _token0, ) = BakerySwapLibrary.sortTokens(_input, _output);
            ...
            address _to = i < _path1.length - 2 ? BakerySwapLibrary.pairFor(elcSwap.factory(), _output, _path1[i + 2]) : address(this);
            IBakerySwapPair(BakerySwapLibrary.pairFor(elcSwap.factory(), _input, _output)).swap(_amount0Out, _amount1Out, _to);
        }
    ...
    function contractionComputeElpNeed() internal view returns(uint256) {      
        ...
        (address _token0, ) = BakerySwapLibrary.sortTokens(address(elc), address(usdt));
        ...
        (_token0, ) = BakerySwapLibrary.sortTokens(address(elp), address(usdt));
        ...
        return BakerySwapLibrary.getAmountIn(_usdtNeed, _reserveELP, _reserveUSDT2);
    }
